...
describe("inventory reconciliation", () => {
  it("reports reviewed sample inventory and flags the missing Tier 1 reserve", () => {
    const result = reconcileInventory(browserSeededInventory);

    expect(result.summary).toBe("needs_review");
    expect(result.issues).toEqual(
      expect.arrayContaining([expect.objectContaining({ id: "empty-tier-pool:1", severity: "warning" })])
    );
