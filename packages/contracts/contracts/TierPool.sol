...
contract TierPool is AccessControl, ERC1155Holder, Pausable, ReentrancyGuard {
    ...
    function reserveClaim(
        ...
        if (eligible < required) revert InsufficientPoolInventory(poolKey, required, eligible);
        ...
        returns (uint256[] memory candidates)
    {
        ...
        for (uint256 optionIndex = 0; optionIndex < reservation.optionCount; optionIndex++) {
            uint256 eligible = _eligibleCount(reservation.poolKey, reservation.excludedCanonicalKey);
            if (eligible == 0) {
                revert InsufficientPoolInventory(reservation.poolKey, 1, 0);
            }
            uint256 selectedEligibleIndex = randomness % eligible;
            uint256 tokenId = _eligibleTokenAt(
                reservation.poolKey,
    ...
    function _eligibleCount(bytes32 poolKey, bytes32 excludedCanonicalKey) private view returns (uint256 count) {
        uint256[] storage tokens = _poolTokens[poolKey];
        for (uint256 index = 0; index < tokens.length; index++) {
            CollectibleForgePolicy.TokenPolicy memory policy = collectiblePolicy.getTokenPolicy(tokens[index]);
            if (excludedCanonicalKey == bytes32(0) || policy.canonicalKey != excludedCanonicalKey) count++;
        }
    }
        ...
        returns (bytes32 poolKey, bytes32 setKey)
    {
        ...
        if (_poolTokens[poolKey].length >= MAX_POOL_TOKENS) revert PoolCapacityReached(poolKey);
    ...
    function _eligibleTokenAt(bytes32 poolKey, bytes32 excludedCanonicalKey, uint256 targetIndex)
        private
        view
        returns (uint256 tokenId)
    {
        ...
        for (uint256 index = 0; index < tokens.length; index++) {
            ...
            CollectibleForgePolicy.TokenPolicy memory policy = collectiblePolicy.getTokenPolicy(candidate);
            if (excludedCanonicalKey != bytes32(0) && policy.canonicalKey == excludedCanonicalKey) continue;
